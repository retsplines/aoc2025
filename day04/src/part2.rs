fn main() {
    ...
    // Loop over the puzzle, removing (setting to false) any movable rolls
    loop {

        // Make a copy of the current layout
        let mut rows_with_removals = rows.clone();
        let mut removals = 0;
        ...
        for (row_index, row) in rows.iter().enumerate() {
            ...
            'roll: for (col_index, &col) in row.iter().enumerate() {
                ...
                rows_with_removals[row_index][col_index] = false;
                removals += 1;
            }
        }

        if removals > 0 {
            println!("This round removed {}", removals);
            total_removals += removals;

            // Update the board with the removed items
            rows = rows_with_removals.clone();

        } else {
            println!("No more removals. Total {}", total_removals);
            break;
        }
    }
}
